...
\marginnote{\textbf{\textsf{Classification trees were hugely popular in the early years of machine learning, when they were first independently proposed by the engineer Ross Quinlan (C4.5) and a group of statisticians (CART), including the father of random forests Leo Brieman.}}}
...
\begin{figure}[h]
    ...
    \caption{\textbf{\textsf{Careful, the \widget{Data} widget needs to be connected to the \widget{Scatter Plot}'s \textit{Data} input, and \widget{Tree Viewer} to the \widget{Scatter Plot}'s \textit{Data Subset} input.}}}
\end{figure}
...
\begin{figure*}[h]
  ...
  \caption{\textbf{\textsf{In the \widget{Tree Viewer} we selected the rightmost node. All data instances coming to the selected node are highlighted in \widget{Scatter Plot}.}}}
\end{figure*}
